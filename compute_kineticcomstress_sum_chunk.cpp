...
#include <cstring>
#include <cmath>
#include "compute_kineticcomstress_sum_chunk.h"
#include "neighbor.h"
#include "atom.h"
#include "update.h"
#include "comm.h"
...
ComputeKineticcomstressSumChunk::ComputeKineticcomstressSumChunk(LAMMPS *lmp, int narg, char **arg) :
  ...
  id_temp(NULL), stress(nullptr),stress_all(nullptr), massproc(nullptr), masstotal(nullptr), vcm(nullptr), vcmall(nullptr)
{
  ...
  ComputeKineticcomstressSumChunk::init();
  ComputeKineticcomstressSumChunk::allocate();
}

/* ---------------------------------------------------------------------- */

ComputeKineticcomstressSumChunk::~ComputeKineticcomstressSumChunk()
{
  delete [] id_temp;
...
void ComputeKineticcomstressSumChunk::allocate()
{
  ...
  maxchunk = nchunk;
  if (maxchunk == 0) error->all(FLERR, "No chunks found for kineticcomstresssum/chunk");

  memory->create(stress,9,"kineticcomstresssum/chunk:stress");
  memory->create(stress_all,9,"kineticcomstresssum/chunk:stress_all");
  memory->create(massproc, maxchunk, "kineticcomstresssum/chunk:massproc");
  memory->create(masstotal, maxchunk, "kineticcomstresssum/chunk:masstotal");
  memory->create(vcm, maxchunk, 3, "kineticcomstresssum/chunk:vcm");
  memory->create(vcmall, maxchunk, 3, "kineticcomstresssum/chunk:vcmall");
  vector=stress_all;
}
...
void ComputeKineticcomstressSumChunk::setup()
{
...
void ComputeKineticcomstressSumChunk::compute_vector()
{
  int index;
  double massone;
